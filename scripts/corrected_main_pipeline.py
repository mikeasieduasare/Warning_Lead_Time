# =================================================================
# CORRECTED RESEARCH PIPELINE  (v3)
#
# Changes vs. previous version, per explicit request:
#   1. Safety Shell (conformal expansion + detrended-AC1 EWS agreement)
#      is now actually built and evaluated: Blocked/Silent failure counts,
#      compared against MSP / Energy / Mahalanobis with paired bootstrap
#      significance testing -- not just raw ingredient data collected.
#   2. AC1 is now DETRENDED before windowed computation (Gaussian-filter
#      residual), per the citation-backed EWS literature standard.
#   3. Each architecture is fine-tuned with 3 INDEPENDENT SEEDS. Reports
#      both raw per-seed values (val_accuracy, alpha_star) AND a pooled
#      summary combining training-stochasticity variance with panel-
#      resampling (bootstrap) variance into one CI.
#   4. Threshold sensitivity analysis: alpha*/lead_time recomputed across
#      tau in {0.65, 0.70, 0.75, 0.80}, reusing cached matrices (no
#      re-inference), addressing "why tau=0.70 specifically."
#   5. All tables exported as CSV + Markdown; all figures are clean,
#      multi-seed-aware, publication-styled (see publication_utils.py).
#
# VERIFICATION BOUNDARY: every pure numpy/scipy function this file calls
# (detrending, onset fusion, blocked/silent counting, bootstrap CIs,
# significance tests, threshold sensitivity, pooling) was unit-tested with
# synthetic data during development and confirmed correct. The torch/GPU-
# dependent parts of THIS file (training loops, forward passes, feature
# hooks) cannot be executed in this environment (no GPU, no torch
# installed here) and have not been end-to-end run -- only checked for
# syntax correctness and logical consistency with the already-verified
# statistical functions they call. Treat the first Kaggle run as the real
# verification of the torch-dependent path.
#
...
from cross_domain_shift import RSNATargetPool, assign_fixed_partners, fda_cross_domain_shift
...
RSNA_POOL_SIZE = 500
MC_DROPOUT_SAMPLES = 10         # smoke-test lever: this multiplies inference cost heavily
                                 # (10 extra stochastic forward passes PER BATCH, at every alpha,
                                 # for every architecture/seed) and is untouched by TRAIN_EPOCHS/
                                 # SEEDS overrides. Lower for a fast smoke test.
FDA_BETA = 0.10
TRAIN_EPOCHS = 10
TRAIN_LR = 1e-4
...
def apply_clinical_shift(images, target_partners, alpha, to_unit, to_model_space, beta=FDA_BETA):
    """Real cross-domain shift via FDA low-frequency amplitude blending."""
        ...
        for alpha in tqdm(alphas, desc=f"{arch_name} alpha sweep"):
            ...
            for inputs, labels in loader:
                ...
                with torch.no_grad():
                    ...
                    # MSP/Energy computed directly from logits/probs already
                    # obtained above -- NOT via compute_baseline_scores(model,
                    # inputs), which would redo the entire forward pass. This
                    # alone was doubling the non-MC-Dropout inference cost of
                    # every batch for no benefit.
...
def analyze_one_run(arch_name, seed, model, n_classes, train_ds, val_ds, test_ds,
    ...
    # --- False-alarm-rate analysis: does the Shell's conservatism (requiring
    # convergent evidence from 2 independent families) actually reduce
    # spurious alerts compared to any single detector alone? Reuses the same
    # signals already computed above -- see false_alarm_rate_analysis's
    # docstring for the full surrogate-null methodology. ---
    ...
    )  # dynamical_family_composite_evidence left at its conservative default (False):
       # injecting the real run's composite evidence into a noise-only surrogate
       # test would mix real and null data, so this test evaluates the
       # dynamical family via AC1's own crossing only (a documented lower-bound
       # on the Shell's true false-alarm rate -- see function docstring).
...
def build_resumed_run_stub(arch_name, seed, signals_dict, threshold=FAILURE_THRESHOLD, thresholds_grid=THRESHOLD_GRID):
    ...
    signals (accuracy curve, detrended AC1, and the four baseline means)
    -- used to RESUME an interrupted session without re-training or
    re-collecting completed (architecture, seed) runs.

    WHAT IS GENUINELY RECOVERABLE from the saved mean signals alone
    (all cheap, pure-numpy recomputation): alpha_star, alpha_ews,
    lead_time, Safety Shell onset/family-vote, composite EWS agreement,
    false-alarm rates, and per-threshold POINT estimates (alpha_star,
    lead_time at each tau in thresholds_grid).

    WHAT IS NOT RECOVERABLE (the saved files only contain already-
    averaged per-alpha means, not the full per-image matrices needed to
    resample them) and is explicitly marked NaN rather than silently
    omitted: bootstrap confidence intervals, blocked/silent failure
    counts, Shell-vs-baseline paired significance, and val_accuracy
    (not saved in the signals dict at all prior to this fix). Pooling
    functions (pool_across_seeds) already drop NaN entries before
    computing statistics, so a resumed run's missing bootstrap
    contributes nothing to pooled CIs rather than corrupting them --
    pooled results for architectures with resumed seeds simply reflect
    fewer effective seeds for that specific statistic.
    """
    alphas = np.asarray(signals_dict["alphas"])
    accuracy = np.asarray(signals_dict["accuracy"])
    ...
    metrics = calculate_transition_metrics(alphas, accuracy, ac1, threshold=threshold)
    ...
    for tau in thresholds_grid:
        tm = calculate_transition_metrics(alphas, accuracy, ac1, threshold=tau)
    ...
    # Fake traj/ews matrices: reshaping each saved (n_alphas,) mean array to
    # (n_alphas, 1) makes any downstream `.mean(axis=1)` call (used
    # throughout the figures/plotting code) exactly reproduce the original
    # mean -- a compatible stand-in without needing the real per-image data.
    fake_traj = {
        "alphas": alphas,
        "correct_matrix": accuracy.reshape(-1, 1),
        "setsize_matrix": setsize_means.reshape(-1, 1),
        "msp_matrix": msp_means.reshape(-1, 1),
        "energy_matrix": energy_means.reshape(-1, 1),
        "mahalanobis_matrix": maha_means.reshape(-1, 1),
        "mc_dropout_var_matrix": mc_means.reshape(-1, 1),
    }

    return {
        "arch_name": arch_name, "seed": seed, "val_acc": float("nan"),  # not saved prior to this fix -- genuinely unrecoverable
        "traj": fake_traj, "ews": ews, "metrics": metrics, "ac1_onset": ac1_onset, "conformal_onset": conformal_onset,
...
def main(output_dir=None, seeds=None, thresholds=None):
    ...
    for arch_name in ARCHITECTURES:
        for seed in seeds:
            raw_signals_path = os.path.join(output_dir, f"raw_signals_{arch_name}_seed{seed}.npz")
            if os.path.isfile(raw_signals_path):
                print(f"\n{'='*60}\n{arch_name} | seed {seed}  [RESUMING FROM SAVED SIGNALS -- "
                      f"skipping retraining; bootstrap CIs, blocked/silent counts, and val_accuracy "
                      f"for this run will be NaN, see build_resumed_run_stub docstring]\n{'='*60}")
                cached = np.load(raw_signals_path)
                signals_dict = {k: cached[k] for k in cached.files}
                run = build_resumed_run_stub(arch_name, seed, signals_dict)
                ...
                signals_dict = {
                    ...
                    "val_accuracy": run["val_acc"],  # now saved -- was missing before, blocking full resume
                    ...
                    # PER-IMAGE MATRICES (spec v3 S10): required for the post-hoc
                    # calibrated analysis (hierarchical bootstrap, joint false-alarm
                    # null, CSD audit). Shape (n_alpha, n_panel). Saving these makes
                    # the authoritative analysis recomputable without retraining.
                ...
                save_raw_signals_npz(output_dir, arch_name, seed, signals_dict)
                raw_signals[arch_name].append(signals_dict)

            m, comp = run["metrics"], run["composite"]
            per_seed_rows.append({
                "Model": arch_name, "seed": seed, "val_accuracy": run["val_acc"],
                "resumed_from_cache": run.get("resumed_from_cache", False),
    ...
    export_table(per_seed_df, output_dir, "table_per_seed_detail")
    ...
    export_table(agg_df, output_dir, "table_pooled_summary")
    ...
    export_table(pairwise_df, output_dir, "table_pairwise_architecture_significance")
    ...
    export_table(shell_comparison_df, output_dir, "table_safety_shell_vs_baselines")

    # =================================================================
    # FALSE-ALARM-RATE COMPARISON: does the Shell's conservatism (requiring
    # convergent evidence from 2 independent families) actually reduce
    # spurious alerts, or is it strictly worse than the best individual
    # detector on every axis? Averaged across the 3 seeds per architecture.
    # =================================================================
    far_rows = []
    for arch_name in ARCHITECTURES:
        runs = runs_by_arch[arch_name]
    ...
    export_table(false_alarm_df, output_dir, "table_false_alarm_rates")
    ...
    export_table(threshold_df, output_dir, "table_threshold_sensitivity")
    ...
    fig1.savefig(os.path.join(output_dir, "fig_accuracy_and_ac1.png"))
    fig2.savefig(os.path.join(output_dir, "fig_threshold_sensitivity.png"))
    fig3.savefig(os.path.join(output_dir, "fig_detector_comparison.png"))
    ...
    fig4.savefig(os.path.join(output_dir, "fig_pairwise_significance.png"))
    ...
    fig5.savefig(os.path.join(output_dir, "fig_SUMMARY_1_reliability_trajectories.png"))
    ...
    fig6.savefig(os.path.join(output_dir, "fig_SUMMARY_2_detector_effectiveness.png"))

    # Third comparison figure: false-alarm rate per detector, same visual
    # form as Figure 2 (reused directly) but a clearly distinct filename/
    # framing -- LOWER is better here, opposite of the blocked-fraction plot.
    ...
    fig7.savefig(os.path.join(output_dir, "fig_false_alarm_rate_comparison.png"))

# =================================================================
# EXPERIMENT 2 PIPELINE: HISTOPATHOLOGY (PCam -> BreaKHis)
#
# Cross-modality validation of the Warning Lead Time framework:
...
#     used in place of WILDS Camelyon17 specifically to avoid its ~10GB
#     download for a weekly iteration cycle -- see BreakHisTargetPool's
#     docstring in histopath_data.py for the full, honest tradeoff this
#     involves: a real, independently-collected dataset and acquisition
#     pipeline, but not WILDS's explicit named-multi-hospital design.
#     WildsCamelyon17TargetPool remains available as a documented
#     fallback if bandwidth/time allow the full download later.)
#   - Task: binary tumor/normal patch classification
#   - Shift mechanism: SAME Fourier Domain Adaptation blend as Experiment
#     1 (imported directly from cross_domain_shift.py, unmodified)
#   - Headline metric: AUC, not accuracy (agreed with PI) -- every other
#     piece of the framework (Safety Shell, MSP/Energy/Mahalanobis/MC
#     Dropout baselines, bootstrap CIs, threshold sensitivity, multi-seed
#     pooling, significance testing) is reused UNCHANGED from
#     enhanced_ews_pipeline.py; only the accuracy->AUC metric functions
#     are new (compute_alpha_indexed_auc, threshold_sensitivity_analysis_auc).
#
# VERIFICATION BOUNDARY: identical to corrected_main_pipeline.py -- all
# pure numpy/scipy logic (AUC computation, threshold sensitivity, the
# stratified PCam split, the BreakHisTargetPool recursive file scan) has
# been unit-tested with synthetic data. The torch/GPU-dependent path
# (training, forward passes) cannot be executed in this sandbox (no GPU,
# no torch, no network to fetch Kaggle data) and has not been end-to-end
# run. Treat your first Kaggle run as the real verification of that path.
#
# REQUIRES:
...
from cross_domain_shift import assign_fixed_partners, fda_cross_domain_shift
...
FDA_BETA = 0.10                 # unchanged from Experiment 1 -- same shift mechanism, deliberately
TRAIN_EPOCHS = 10
TRAIN_LR = 1e-4
...
def apply_clinical_shift(images, target_partners, alpha, to_unit, to_model_space, beta=FDA_BETA):
    """Identical shift mechanism to Experiment 1."""
    unit_images = to_unit(images)
    shifted = fda_cross_domain_shift(unit_images, target_partners, alpha, beta=beta)
...
def analyze_one_run(arch_name, seed, model, n_classes, train_ds, val_ds, test_ds,
    ...
    raw_signals_path = os.path.join(
        ...
        f"exp2_raw_signals_{arch_name}_seed{seed}.npz"
    )
...
def main(output_dir=None, seeds=None, thresholds=None):
    ...
    for arch_name in ARCHITECTURES:
        for seed in seeds:
            ...
            save_raw_signals_npz(output_dir, arch_name, seed, signals_dict, prefix="exp2_")
    ...
    export_table(per_seed_df, output_dir, "exp2_table_per_seed_detail")
    ...
    export_table(agg_df, output_dir, "exp2_table_pooled_summary")
    ...
    export_table(pairwise_df, output_dir, "exp2_table_pairwise_architecture_significance")
    ...
    export_table(shell_comparison_df, output_dir, "exp2_table_safety_shell_vs_baselines")
    ...
    export_table(false_alarm_df, output_dir, "exp2_table_false_alarm_rates")
    ...
    export_table(threshold_df, output_dir, "exp2_table_threshold_sensitivity")
    ...
    fig1.savefig(os.path.join(output_dir, "exp2_fig_auc_and_ac1.png"))
    fig2.savefig(os.path.join(output_dir, "exp2_fig_threshold_sensitivity.png"))
    fig3.savefig(os.path.join(output_dir, "exp2_fig_detector_comparison.png"))
    ...
    fig4.savefig(os.path.join(output_dir, "exp2_fig_pairwise_significance.png"))
    ...
    fig5.savefig(os.path.join(output_dir, "exp2_fig_SUMMARY_1_reliability_trajectories.png"))
    ...
    fig6.savefig(os.path.join(output_dir, "exp2_fig_SUMMARY_2_detector_effectiveness.png"))
    ...
    fig7.savefig(os.path.join(output_dir, "exp2_fig_false_alarm_rate_comparison.png"))

    plt.show()
    print(f"\nAll Experiment 2 tables and figures saved to: {output_dir}")
    print(f"Raw per-run signals saved as exp2_raw_signals_<arch>_seed<N>.npz in {output_dir} "
          f"(recoverable via publication_utils.load_raw_signals(..., prefix='exp2_') even after a lost session).")

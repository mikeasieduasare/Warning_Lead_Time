# =================================================================
# EXPERIMENT 2 (LINEAR-SHIFT VARIANT): HISTOPATHOLOGY (PCam -> BreaKHis)
#
# Parallel variant of the histopathology cross-modality experiment, using
# the PIXEL-SPACE LINEAR BLEND shift mechanism instead of Fourier Domain
# Adaptation. Run ALONGSIDE corrected_main_pipeline_histopath.py (the FDA
# variant), not instead of it, to test whether the framework's conclusions
# on the histopathology domain depend on the shift MECHANISM -- the same
# question the linear-shift RSNA variant (corrected_main_pipeline_
# linearshift.py) asks for the radiology domain. Directly addresses the
# "single shift mechanism" limitation for Experiment 2.
#   - Source: PatchCamelyon (Kaggle "Histopathologic Cancer Detection")
#   - Target: BreaKHis breast-tumor histopathology (single-lab collection;
#     see BreakHisTargetPool docstring in histopath_data.py for the full
#     tradeoff. WildsCamelyon17TargetPool remains a documented fallback.)
#   - Task: binary tumor/normal patch classification
#   - Shift mechanism: pixel-space linear blend, x_alpha = (1-alpha)*x_source
#     + alpha*x_target (linear_pixel_blend_shift in cross_domain_shift.py).
#     A literal, monotonic mixture -- see that function's docstring for why
#     it may produce cleaner critical-slowing-down precursors than FDA, and
#     the face-validity tradeoff it makes at intermediate alpha.
#   - Headline metric: AUC, not accuracy -- every other piece of the
#     framework (Safety Shell, MSP/Energy/Mahalanobis/MC Dropout baselines,
#     bootstrap CIs, threshold sensitivity, multi-seed pooling, significance
#     testing) is reused UNCHANGED from enhanced_ews_pipeline.py.
#   - All outputs use an "exp2lin_" filename prefix so they coexist with the
#     FDA histopath experiment's "exp2_" outputs without collision.
#
# VERIFICATION BOUNDARY: identical to corrected_main_pipeline_histopath.py
# -- all pure numpy/scipy logic has been unit-tested with synthetic data,
# and the linear blend mechanism itself was verified numerically (exact
# midpoint match, proportional deviation at every alpha). The torch/GPU-
# dependent path (training, forward passes) cannot be executed in this
# sandbox and has not been end-to-end run. Treat your first Kaggle run as
# the real verification of that path.
#
# REQUIRES:
#   A BreaKHis dataset added as a Kaggle notebook input (search "breakhis"
#   in Kaggle Datasets -- several mirrors exist under ~1GB). Check the
#   actual mount path with `!ls /kaggle/input/` and update BREAKHIS_ROOT
#   below if it doesn't match the default.
#   Kaggle competition dataset "Histopathologic Cancer Detection" added
#   as a notebook input (provides PCam source images + labels).
# =================================================================
...
from histopath_data import get_pcam_data, BreakHisTargetPool
from cross_domain_shift import assign_fixed_partners, linear_pixel_blend_shift
...
from enhanced_ews_pipeline import (
    ...
    compute_alpha_indexed_auc,
    threshold_sensitivity_analysis_auc,
...
from publication_utils import (
    ...
    plot_summary_figure_2_detector_effectiveness, save_raw_signals_npz,
)
...
PANEL_SIZE = 400
N_BOOT = 500
N_BOOT_THRESHOLD = 100
FAILURE_THRESHOLD = 0.70        # AUC threshold now, not accuracy -- justified in the paper as a
                                 # recognized "acceptable discrimination" cutoff, not a reused number
THRESHOLD_GRID = (0.65, 0.70, 0.75, 0.80)
SEEDS = (0, 1, 2)
SEED = 42
PCAM_ROOT = "/kaggle/input/competitions/histopathologic-cancer-detection"
# ^ Competitions in this Kaggle environment mount under /kaggle/input/competitions/<slug>,
# not directly under /kaggle/input/<slug> -- confirmed via the RSNA path issue in
# Experiment 1 and this same issue recurring for the PCam competition. Still worth
# a quick `!ls /kaggle/input/competitions/` if this ever changes on Kaggle's end.
BREAKHIS_ROOT = "/kaggle/input/datasets/waseemalastal/breakhis-breast-cancer-histopathological-dataset"
# ^ Confirmed pattern in this Kaggle environment: regular Datasets mount at
# /kaggle/input/datasets/<username>/<slug>, while Competitions mount at
# /kaggle/input/competitions/<slug> (see PCAM_ROOT above) -- two different
# conventions for two different input types. Still worth a quick
# `!ls /kaggle/input/datasets/waseemalastal/` to confirm if this ever changes.
# ^ matches https://www.kaggle.com/datasets/waseemalastal/breakhis-breast-cancer-histopathological-dataset
# Kaggle mounts datasets at /kaggle/input/<slug>, where <slug> is the part of the
# URL after the username. Still worth a quick `!ls /kaggle/input/` after adding it,
# since Kaggle occasionally nests one extra folder level for multi-file dataset uploads.
BREAKHIS_POOL_SIZE = 500
MC_DROPOUT_SAMPLES = 10         # smoke-test lever: see corrected_main_pipeline.py for rationale
TRAIN_EPOCHS = 10
TRAIN_LR = 1e-4
MIN_ACCEPTABLE_VAL_AUC = 0.85   # gate: PCam is an easier task than CXR pneumonia; a well-trained
                                 # model should clear this comfortably. Raise EPOCHS if not.
...
POSITIVE_CLASS_INDEX = 1        # label convention: 1 = tumor, 0 = normal (standard PCam convention)
...
def get_denormalize_renormalize():
    """Same formula as Experiment 1 -- works identically for RGB since
    PCam/BreaKHis also use a symmetric mean=.5/std=.5 normalization."""
    def to_unit(x):
...
def build_target_partners(panel_size, device, root=BREAKHIS_ROOT,
                           pool_size=BREAKHIS_POOL_SIZE, seed=SEED):
    """Fixed BreaKHis target pool + fixed partner assignment,
    reused across every architecture and seed."""
    pool = BreakHisTargetPool(root=root, pool_size=pool_size, seed=seed)
...
def apply_clinical_shift(images, target_partners, alpha, to_unit, to_model_space):
    ...
    in cross_domain_shift.py. Parallel variant to the FDA histopath experiment,
    testing whether the shift mechanism affects the framework's conclusions on
...
# =================================================================
# ARCHITECTURES  (identical to Experiment 1 -- num_classes=2 either way)
# =================================================================
def initialize_single_model(arch_name, num_classes):
    """Builds ONE architecture by name. A Dropout layer (p=0.2) is inserted
    before the final classifier for EVERY architecture, uniformly -- this
    is what makes MC Dropout (Gal & Ghahramani, 2016) usable and
    COMPARABLE across all three, since torchvision's ResNet18/DenseNet121
    have no dropout by default (unlike ViT-B16)."""
...
# =================================================================
# FINE-TUNING  (AUC-based validation gate instead of accuracy)
# =================================================================
def train_source_model(model, train_ds, val_ds, epochs=TRAIN_EPOCHS, lr=TRAIN_LR,
                        device=DEVICE, batch_size=BATCH_SIZE, min_acceptable_auc=MIN_ACCEPTABLE_VAL_AUC,
                        name="", max_train_samples=None, max_val_samples=None):
    """
    `max_train_samples`/`max_val_samples`: if set, trains/validates on a
    random subset of this size instead of the full split. TRAIN_EPOCHS
    alone does NOT control runtime for a smoke test -- PCam's training
    split is ~187,000 images, so even 2 epochs is still 2 full passes
    over that many images. Use these caps for a genuinely fast smoke
    test; leave them None (full dataset) for the real run.
    """
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for inputs, labels in val_loader:
                inputs = inputs.to(device)
                probs = torch.softmax(model(inputs), dim=1)[:, POSITIVE_CLASS_INDEX]
                all_probs.extend(probs.cpu().numpy())
                all_labels.extend(labels.numpy())
        val_auc = roc_auc_score(all_labels, all_probs)

        print(f"[{name}] epoch {epoch+1}/{epochs} | train_loss={running_loss/len(train_ds):.4f} | val_auc={val_auc:.4f}")
    ...
    if best_val_auc < min_acceptable_auc:
        raise RuntimeError(
            f"[{name}] best validation AUC ({best_val_auc:.4f}) never reached "
            f"{min_acceptable_auc}. Increase TRAIN_EPOCHS, lower TRAIN_LR, or inspect "
            f"the training curve above before proceeding."
        )

    print(f"[{name}] fine-tuning complete. Best val_auc = {best_val_auc:.4f}")
    return model, best_val_auc


# =================================================================
# FIXED-PANEL TRAJECTORY COLLECTION  (adds prob_matrix for AUC)
# =================================================================
def collect_panel_trajectories(model, arch_name, mahalanobis_params, test_ds, panel_indices,
    ...
    """
    Same design as Experiment 1, PLUS a prob_matrix (predicted probability
    of the positive/tumor class per sample per alpha) needed to compute
    AUC, and an mc_dropout_var_matrix for the MC Dropout baseline.
    correct_matrix (argmax-based) is still collected too, since the
    Safety Shell's blocked/silent counting operates on individual
    misclassifications regardless of which headline metric is used.
    """
    ...
    entropy_matrix, correct_matrix, setsize_matrix, prob_matrix = [], [], [], []
    ...
    model.eval()
    try:
        for a_idx, alpha in enumerate(tqdm(alphas, desc=f"{arch_name} alpha sweep")):
            step_entropy, step_correct, step_setsize, step_prob = [], [], [], []
            ...
            for inputs, labels in loader:
                ...
                step_prob.extend(probs[:, POSITIVE_CLASS_INDEX].cpu().numpy())
                step_msp.extend(msp)
                step_energy.extend(energy)
                step_maha.extend(maha)
                step_mc_var.extend(mc_var)

                if a_idx == 0:  # panel labels are fixed -- only need to record once
                    panel_labels.extend(labels.cpu().numpy())

            entropy_matrix.append(step_entropy)
            correct_matrix.append(step_correct)
            setsize_matrix.append(step_setsize)
            prob_matrix.append(step_prob)
    ...
    return {
        ...
        "panel_labels": np.array(panel_labels),
    }


# =================================================================
# PER-(ARCHITECTURE, SEED) ANALYSIS  (AUC in place of accuracy)
# =================================================================
def build_resumed_run_stub(arch_name, seed, signals_dict, threshold=FAILURE_THRESHOLD, thresholds_grid=THRESHOLD_GRID):
    """
    AUC-based counterpart to Experiment 1's build_resumed_run_stub --
    reconstructs a run-dict compatible object from ONLY the saved raw
    signals, to RESUME an interrupted session without re-training or
    re-collecting completed (architecture, seed) runs.

    Recoverable from saved means alone: alpha_star, alpha_ews, lead_time,
    Safety Shell onset/family-vote, composite EWS agreement, false-alarm
    rates, per-threshold point estimates. NOT recoverable (needs the full
    per-image matrices, never saved): bootstrap CIs, blocked/silent
    counts, Shell-vs-baseline significance, and val_auc (unless this run
    was saved after the val_auc persistence fix -- older cached files
    from before that fix will show NaN here).
    """
    alphas = np.asarray(signals_dict["alphas"])
    auc_curve = np.asarray(signals_dict["auc_curve"])
    ac1 = np.asarray(signals_dict["ac1_detrended"])
    setsize_means = np.asarray(signals_dict["setsize_means"])
    msp_means = np.asarray(signals_dict["msp_means"])
    energy_means = np.asarray(signals_dict["energy_means"])
    maha_means = np.asarray(signals_dict["mahalanobis_means"])
    mc_means = np.asarray(signals_dict["mc_dropout_means"])
    val_auc = float(signals_dict["val_auc"]) if "val_auc" in signals_dict else float("nan")

    metrics = calculate_transition_metrics(alphas, auc_curve, ac1, threshold=threshold)
    ac1_onset = detect_sustained_onset(alphas, ac1)
    ews = {"ac1": ac1}
    composite = composite_ews_agreement(alphas, ews, ac1_onset["onset_idx"])

    onset_baselines = baseline_onset_detection(alphas, msp_means, energy_means, mahalanobis_means=maha_means,
                                                extra_signals={"mc_dropout": mc_means})
    ...
    calibration_independent_onsets = {"mahalanobis": onset_baselines["mahalanobis_onset"],
    ...
    false_alarm_rates = false_alarm_rate_analysis(
        alphas, {"ac1": ac1, "setsize": setsize_means, "msp": msp_means, "energy": energy_means,
                 "mahalanobis": maha_means, "mc_dropout": mc_means},
        n_surrogates=200, seed=SEED + seed,
    )

    nan_detector_counts = {"blocked": 0, "silent": 0, "total_failures": 0, "blocked_fraction": float("nan")}
    detector_counts = {d: dict(nan_detector_counts) for d in ["SafetyShell", "MSP", "Energy", "Mahalanobis", "MCDropout"]}
    nan_sig = {"observed_diff": float("nan"), "ci95": (float("nan"), float("nan")), "p_value": float("nan"),
               "significant": False, "blocked_fraction_a_mean": float("nan"), "blocked_fraction_b_mean": float("nan"),
               "frac_a_samples": np.array([float("nan")]), "frac_b_samples": np.array([float("nan")])}
    shell_vs_baseline_sig = {d: dict(nan_sig) for d in ["MSP", "Energy", "Mahalanobis", "MCDropout"]}

    threshold_rows = []
    for tau in thresholds_grid:
        tm = calculate_transition_metrics(alphas, auc_curve, ac1, threshold=tau)
        threshold_rows.append({
            "threshold": tau, "alpha_star": tm["alpha_star"], "alpha_star_sustained": tm["alpha_star_sustained"],
            "alpha_star_ci95": (float("nan"), float("nan")), "lead_time": tm["lead_time"],
            "lead_time_ci95": (float("nan"), float("nan")),
            "boot_alpha_star_samples": [float("nan")], "boot_lead_samples": [float("nan")],
        })

    return {
        "arch_name": arch_name, "seed": seed, "val_auc": val_auc,
        "traj": {"alphas": alphas}, "ews": ews, "metrics": metrics, "ac1_onset": ac1_onset,
        "conformal_onset": conformal_onset, "auc_curve": auc_curve,
        "composite": composite, "baseline_lead": baseline_lead,
        "baseline_auroc": {"msp_auroc": float("nan"), "energy_auroc": float("nan"),
                            "mahalanobis_auroc": float("nan"), "mc_dropout_auroc": float("nan")},
        ...
        "boot_alpha_star": [float("nan")], "boot_lead": [float("nan")], "boot_shell_lead": [float("nan")],
        "threshold_rows": threshold_rows, "resumed_from_cache": True,
    }
...
def analyze_one_run(arch_name, seed, model, n_classes, train_ds, val_ds, test_ds,
    ...
    raw_signals_path = os.path.join(
        ...
        f"exp2lin_raw_signals_{arch_name}_seed{seed}.npz"
    )
    if os.path.isfile(raw_signals_path):
        print(f"[RESUMING] {arch_name} seed {seed}: found cached signals at {raw_signals_path}, skipping retraining.")
        cached = np.load(raw_signals_path)
        signals_dict = {k: cached[k] for k in cached.files}
        return build_resumed_run_stub(arch_name, seed, signals_dict)

    torch.manual_seed(seed)
    np.random.seed(seed)

    model, val_auc = train_source_model(
        model, train_ds, val_ds, epochs=TRAIN_EPOCHS, lr=TRAIN_LR,
        min_acceptable_auc=MIN_ACCEPTABLE_VAL_AUC,
    ...
    alphas = traj["alphas"]
    entropy_matrix, correct_matrix = traj["entropy_matrix"], traj["correct_matrix"]
    prob_matrix, panel_labels = traj["prob_matrix"], traj["panel_labels"]
    setsize_matrix = traj["setsize_matrix"]

    state_series = entropy_matrix.mean(axis=1)
    auc_curve = compute_alpha_indexed_auc(prob_matrix, panel_labels)  # <-- AUC replaces accuracy here

    ews = windowed_ews_indicators(state_series)
    metrics = calculate_transition_metrics(alphas, auc_curve, ews["ac1"], threshold=FAILURE_THRESHOLD)
    ...
    # --- Safety Shell: FAMILY-STRATIFIED (same design as Experiment 1).
    # Dynamical family: AC1 sustained crossing, or composite convergent
    # evidence validating it. Calibration-independent family: earliest
    # genuine among {Mahalanobis, Energy, MC Dropout}. Conformal set-size
    # is excluded from voting (calibration-dependent blind spot) but still
    # computed for diagnostic reporting. ---
    ...
    detector_onsets = {
        "SafetyShell": shell["shell_onset_idx"], "MSP": onset_baselines["msp_onset"]["onset_idx"],
        "Energy": onset_baselines["energy_onset"]["onset_idx"], "Mahalanobis": onset_baselines["mahalanobis_onset"]["onset_idx"],
    ...
    # bootstrap CIs (AUC-aware: resample prob_matrix + panel_labels jointly)
    ...
    for _ in range(N_BOOT):
        ...
        b_auc = compute_alpha_indexed_auc(prob_matrix[:, cols], panel_labels[cols])
        b_setsize = setsize_matrix[:, cols].mean(axis=1)
        ...
        bm = calculate_transition_metrics(alphas, b_auc, b_ews["ac1"], threshold=FAILURE_THRESHOLD)
    ...
    threshold_rows = threshold_sensitivity_analysis_auc(
        alphas, entropy_matrix, prob_matrix, panel_labels, thresholds=THRESHOLD_GRID,
...
def main(output_dir=None, seeds=None, thresholds=None):
    # None-sentinel pattern: SEEDS/THRESHOLD_GRID are resolved HERE, inside
    # the function body, so runtime overrides (e.g. exp2.SEEDS = (0,) set
    # AFTER import) actually take effect. Binding them directly as
    # parameter defaults (seeds=SEEDS) would silently freeze whatever
    # SEEDS equaled at IMPORT time -- a real bug that was caught after a
    # "reduced" smoke test silently ran at full scale anyway.
    if seeds is None:
        seeds = SEEDS
    ...
    train_ds, val_ds, test_ds, n_classes = get_pcam_data(root=PCAM_ROOT, seed=SEED)
    ...
    target_partners = build_target_partners(PANEL_SIZE, DEVICE, root=BREAKHIS_ROOT, pool_size=BREAKHIS_POOL_SIZE)
    ...
    raw_signals = {arch: [] for arch in ARCHITECTURES}  # results["raw_signals"] -- diagnostic checks + crash recovery
    ...
    for arch_name in ARCHITECTURES:
        for seed in seeds:
            ...
            # Raw alpha-indexed signals, saved to disk IMMEDIATELY (survives a
            # lost/restarted kernel session) AND kept in-memory as
            # results["raw_signals"] for direct diagnostic checks, e.g.:
            #   results["raw_signals"]["ResNet18"][0]["setsize_means"]
            traj = run["traj"]
            signals_dict = {
                "alphas": traj["alphas"],
                "auc_curve": run["auc_curve"],
                "val_auc": run["val_auc"],  # now saved -- was missing before, blocking full resume
                ...
                "mc_dropout_means": traj["mc_dropout_var_matrix"].mean(axis=1),
                # PER-IMAGE MATRICES (spec v3 S10): required for post-hoc calibrated
                # analysis. Histopath uses AUC, so prob_matrix + panel_labels are
                # saved (AUC-based collapse) alongside correct_matrix.
                "entropy_matrix": traj["entropy_matrix"],
                "correct_matrix": traj["correct_matrix"],
                "prob_matrix": traj["prob_matrix"],
                "panel_labels": traj["panel_labels"],
            ...
            save_raw_signals_npz(output_dir, arch_name, seed, signals_dict, prefix="exp2lin_")
            raw_signals[arch_name].append(signals_dict)

            m, comp = run["metrics"], run["composite"]
            per_seed_rows.append({
                "Model": arch_name, "seed": seed, "val_auc": run["val_auc"],
    ...
    print("\n" + "=" * 70)
    print("     [EXP 2: HISTOPATHOLOGY] PER-SEED DETAIL TABLE")
    print("=" * 70)
    print(per_seed_df.to_string(index=False))
    export_table(per_seed_df, output_dir, "exp2lin_table_per_seed_detail")

    agg_rows = []
    boot_lead_pool_by_arch = {}
    for arch_name in ARCHITECTURES:
        runs = runs_by_arch[arch_name]
        val_auc_summary = seed_variance_summary([r["val_auc"] for r in runs])
        ...
        agg_rows.append({
            "Model": arch_name,
            "val_auc_mean": val_auc_summary["mean"], "val_auc_std_across_seeds": val_auc_summary["std"],
    ...
    print("\n" + "=" * 70)
    print("     [EXP 2] POOLED SUMMARY ACROSS SEEDS")
    print("=" * 70)
    print(agg_df.to_string(index=False))
    export_table(agg_df, output_dir, "exp2lin_table_pooled_summary")

    unresolved = agg_df[agg_df["n_seeds_with_sustained_collapse"].apply(lambda s: s.startswith("0"))]["Model"].tolist()
    if unresolved:
        print(f"\n*** WARNING: {unresolved} showed NO seed with a sustained AUC drop below "
    ...
    print("\n" + "=" * 70)
    print("     [EXP 2] PAIRWISE ARCHITECTURE SIGNIFICANCE")
    print("=" * 70)
    print(pairwise_df.to_string(index=False))
    export_table(pairwise_df, output_dir, "exp2lin_table_pairwise_architecture_significance")
    ...
    print("\n" + "=" * 70)
    print("     [EXP 2] SAFETY SHELL vs. OOD BASELINES")
    print("=" * 70)
    print(shell_comparison_df.to_string(index=False))
    export_table(shell_comparison_df, output_dir, "exp2lin_table_safety_shell_vs_baselines")
    ...
    print("\n" + "=" * 70)
    print("     [EXP 2] FALSE-ALARM-RATE COMPARISON (surrogate-null test, lower = better)")
    print("=" * 70)
    print(false_alarm_df.to_string(index=False))
    export_table(false_alarm_df, output_dir, "exp2lin_table_false_alarm_rates")
    ...
    print("\n" + "=" * 70)
    print("     [EXP 2] THRESHOLD SENSITIVITY ANALYSIS (AUC-based)")
    print("=" * 70)
    print(threshold_df.to_string(index=False))
    export_table(threshold_df, output_dir, "exp2lin_table_threshold_sensitivity")

    # =================================================================
    # FIGURES  (AUC label swapped in for "Accuracy")
    # =================================================================
    ...
    for i, arch_name in enumerate(ARCHITECTURES):
        ...
        auc_by_seed = [r["auc_curve"] for r in runs]
        plot_accuracy_by_seed(axes1[i, 0], alphas, auc_by_seed, FAILURE_THRESHOLD, f"{arch_name}: AUC vs alpha (3 seeds)")
        axes1[i, 0].set_ylabel("AUC")
        ...
        plot_threshold_sensitivity(axes2[i], threshold_rows_pooled_by_arch[arch_name], f"{arch_name}: tau sensitivity (AUC)")
        ...
        curves_by_arch[arch_name] = np.mean(auc_by_seed, axis=0)
    ...
    fig1.savefig(os.path.join(output_dir, "exp2lin_fig_auc_and_ac1.png"))
    fig2.savefig(os.path.join(output_dir, "exp2lin_fig_threshold_sensitivity.png"))
    fig3.savefig(os.path.join(output_dir, "exp2lin_fig_detector_comparison.png"))
    ...
    fig4.savefig(os.path.join(output_dir, "exp2lin_fig_pairwise_significance.png"))

    # THE two single-glance summary figures -- same minimal, text-free
    # design as Experiment 1: Figure 1 (line plot, AUC replaces accuracy)
    # shows which architectures are fragile vs. robust; Figure 2 (bar
    # chart) shows how well the Safety Shell catches failures vs. every
    # baseline. Different chart forms/palettes so they're immediately
    ...
    plot_summary_figure_1_reliability(
        ...
        curves_by_arch, alpha_star_by_arch, alpha_ews_by_arch, sustained_by_arch,
        threshold=FAILURE_THRESHOLD, metric_name="AUC"
    )
    fig5.savefig(os.path.join(output_dir, "exp2lin_fig_SUMMARY_1_reliability_trajectories.png"))
    ...
    fig6.savefig(os.path.join(output_dir, "exp2lin_fig_SUMMARY_2_detector_effectiveness.png"))
    ...
    fig7.savefig(os.path.join(output_dir, "exp2lin_fig_false_alarm_rate_comparison.png"))

    plt.show()
    print(f"\nAll Experiment 2 tables and figures saved to: {output_dir}")
    print(f"Raw per-run signals saved as exp2lin_raw_signals_<arch>_seed<N>.npz in {output_dir} "
          f"(recoverable via publication_utils.load_raw_signals(..., prefix='exp2lin_') even after a lost session).")
    ...
    # analyze_experiment_posthoc. metric_is_auc=True for this experiment.
    try:
